...
static unsigned char g_logs_table[256] = {
...
static unsigned char g_rev_logs_table[256] = {
...
unsigned char gpoly_interp(int k, unsigned char *xs, unsigned char *ys,
  ...
  for (i = 0; i < k; ++i) {
    ...
    for (j = 0; j < k; ++j) {
      ...
      weight = gmul(weight, gdiv(gsub(x, xs[j]), gsub(xs[i], xs[j])));
    }
    y = gadd(y, gmul(weight, ys[i]));
  }
  return y;
}
...
void gpoly_rev(int k, const unsigned char *xs, const unsigned char *ys,
  ...
  for (i = 0; i < k; ++i) {
    ...
    for (j = 0; j < k; ++j) {
      if (j == i) continue;
      weight = gmul(weight, gsub(xs[i], xs[j]));
      for (l = j + (j < i); l >= 0; --l) {
